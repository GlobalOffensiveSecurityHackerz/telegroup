            ...
            //Add group member in telegram_group_subscribers table when message in group
                ...
                if (isset($raw_response_array['message']['chat']['type']) && !isset($raw_response_array['message']['forward_from']) && ($raw_response_array['message']['chat']['type'] == 'group' || $raw_response_array['message']['chat']['type'] == 'supergroup') ) {
                    ...
                    if(!empty($filter_message_data) && $group_admin != true){
                        ...
                        $keyword_action = $filter_message_data->keyword_action ?? 'nothing';
                            ...
                            if(isset($message['text'])){
                                ...
                                foreach ($wordsToCheck as $word) {
                                    ...
                                    if (strpos($lowercaseText, $lowercaseWord) !== false) {
                                        ...
                                        $chatID = $message['chat']['id'] ?? '';
                                        $messageID = $message['message_id'] ?? '';
                                        $offenderChatId = $message['from']['id'] ?? '';
                                        // Send warning first (reply to offending message)
                                        if ($keyword_action === 'warn') {
                                            $memberName = $message['from']['username'] ?? trim(($message['from']['first_name'] ?? '') . ' ' . ($message['from']['last_name'] ?? ''));
                                            $warningText = '@' . ltrim($memberName, '@') . ' warn by sending restricted keyword.';
                                            if (!empty($matchedWord)) {
                                                $warningText .= ' Keyword: "' . $matchedWord . '".';
                                            }
                                            $warnController = app(\App\Http\Controllers\GroupWarnController::class);
                                            $warnController->sendWarning($bot_token, $chatID, $warningText, $messageID);
                                            // Update warn count
                                            if (!empty($offenderChatId) && !empty($telegram_group_id) && !empty($user_id)) {
                                                $reason = !empty($matchedWord) ? 'keyword: ' . $matchedWord : 'keyword match';
                                                $warnController->incrementWarningLog((int) $user_id, (int) $telegram_group_id, $offenderChatId, $reason);
                                            }
                                        }
                                        // Now delete offending message
                                        $this->telegram->bot_token = $bot_token;
                                        $this->telegram->delete_message($chatID, $messageID);
                                        break;
                                    }
                ...
                if (isset($raw_response_array['message']['forward_from'] )  && $group_admin != true){
                    ...
                    $forwarded_action = $filter_message_data->forwarded_action ?? 'nothing';
                    $offenderChatId = $message['from']['id'] ?? '';
                    $messageID = $message['message_id'] ?? '';
                    $chatID = $message['chat']['id'] ?? '';
                    $forwardReason = 'forwarded message';
                    ...
                    if ($forwarded_action !== 'nothing' && !empty($offenderChatId)) {
                        if ($forwarded_action === 'warn') {
                            $memberName = $message['from']['username'] ?? trim(($message['from']['first_name'] ?? '') . ' ' . ($message['from']['last_name'] ?? ''));
                            $warningText = '@' . ltrim($memberName, '@') . ' warn by forwarding message.';
                            $warnController = app(\App\Http\Controllers\GroupWarnController::class);
                            $warnController->sendWarning($bot_token, $chatID, $warningText, $messageID);
                            if (!empty($telegram_group_id) && !empty($user_id)) {
                                $warnController->incrementWarningLog((int) $user_id, (int) $telegram_group_id, $offenderChatId, $forwardReason);
                            }
                        } elseif (in_array($forwarded_action, ['kick', 'ban'])) {
                            $banController = app(\App\Http\Controllers\GroupBanController::class);
                            $banController->banOrKick($forwarded_action, $bot_token, $chatID, $offenderChatId, $forwardReason, $user_id ?? null);
                            if ($forwarded_action === 'ban') {
                                $memberName = $message['from']['username'] ?? trim(($message['from']['first_name'] ?? '') . ' ' . ($message['from']['last_name'] ?? ''));
                                $banText = '@' . ltrim($memberName, '@') . ' ban by forwarding message.';
                                app(\App\Http\Controllers\GroupWarnController::class)->sendWarning($bot_token, $chatID, $banText, $messageID);
                            }
                        }
                    }
                }
